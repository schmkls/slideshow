"""Single CLI exposing each step as a subcommand.
    ...
    python -m slideshow mask       IN_DIR          [--target T] [--model M]
    python -m slideshow silhouette IN_DIR  OUT_DIR  [--halo-px N] ...
    python -m slideshow center     IN_DIR  OUT_DIR  [--width N] ...
    python -m slideshow fade   IN_DIR OUT_DIR --effect E ...
    python -m slideshow linger IN_DIR OUT_DIR [--no-loop] ...
    python -m slideshow place  IN_DIR OUT_DIR --motion M [--frames N] ...
    python -m slideshow pick-faces   IN_DIR         [--conf C]
...
first. The steps then chain through directories, so they compose: e.g.
mask -> silhouette -> center -> video, or mask -> fade -> video.
``pick-faces`` writes a faces.json that ``mask --target faces`` reads.
"""
...
from . import center, mask, pickfaces, silhouette, video
...
def build_parser() -> argparse.ArgumentParser:
    ...
    m = sub.add_parser(
        "mask", help="Detect each subject once, write its mask beside the photo."
    )
    m.add_argument("input_dir", type=Path)
    m.add_argument("--target", choices=("subject", "faces"), default="subject",
                   help="What to detect: whole subject (rembg) or faces only.")
    m.add_argument("--model", default="u2net",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "mask":
        written = mask.run(
            args.input_dir, target=args.target, model=args.model,
        )
    ...
    if args.command == "pick-faces":
        chosen = pickfaces.run(
            args.input_dir, conf_thresh=args.conf, det_size=args.det_size,
        )
